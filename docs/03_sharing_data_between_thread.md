...
* 线程修改共享数据时，就会发生破坏不变量的情况，此时如果有其他线程访问，就可能导致不变量被永久性破坏，这就是 **竞争条件** (race condition)
* 如果线程执行顺序的先后对结果无影响，则为不需要关心的良性竞争。需要关心的是不变量被破坏时产生的 race condition
* C++ 标准中定义了 **数据竞争** (data race) 的概念，指代一种特定的 race condition，即并发修改单个对象。data race 会造成未定义行为
* race condition 要求一个线程进行时，另一线程访问同一数据块，出现问题时很难复现，因此编程时需要使用大量复杂操作来避免 race condition

* 解决竞争条件的方法：
  * 方法一：（**有锁编程**）将数据结构包裹在某种保护机制下，确保只有进行修改的线程才能看到不变量被破坏时的中间状态。其他线程只能看到开始前和结束后，而无法看到中间状态。
  * 方法二：（**无锁编程**）修改数据结构和不变量的设计，以便让修改由一系列不可分割的变化来完成，每一个都维持了不变量。
  * 方法三：（**本书不讨论**）把更新数据结构当作事务来处理，所需的一系列数据修改和读取存储在一个事务日志中，然后再一步提交。如果提交因为数据结构被另一个线程修改了而不能进行，事务就会被重启。
...
* C++ 17 中可以自动推导模板参数，因此实例化时可以省略模板参数, 不用手动指定类型
  ```cpp
...
* C++17 提供了的 [std::scoped_lock](https://en.cppreference.com/w/cpp/thread/scoped_lock)避免死锁，它可以接受任意数量的 mutex，并将这些 mutex 传给 [std::lock](https://en.cppreference.com/w/cpp/thread/lock) 来同时上锁，它会对其中一个 mutex 调用 lock()，对其他调用 try_lock()，若 try_lock() 返回 false 则对已经上锁的 mutex 调用 unlock()，然后重新进行下一轮上锁，标准未规定下一轮的上锁顺序，可能不一致，重复此过程直到所有 mutex 上锁，从而达到同时上锁的效果。
...
* **切勿将指向保护数据的指针或引用传递到互斥锁 作用域之外**，无论是从**函数返回**它们，还 是把他们**存储在外部可见的内存**，亦或是把他们**以参数形式传递**给用户提供的函数。

  * 一般 mutex 和要保护的数据一起放在类中，定义为 private 数据成员，而非全局变量，这样能让代码更清晰。但如果某个成员函数**返回指向数据成员的指针或引用**，则通过这个指针的访问行为不会被 mutex 限制，因此需要**谨慎设置接口，确保 mutex 能锁住数据**
  ...
  * 除了函数返回值可能泄露数据指针，函数在调用其他函数时，也可能通过指针或引用参数把数据泄露给被调用的函数，尤其是该函数是通过*函数参数*提供时。
    ```cpp
    template<typename Function>
    void process_data(Function func)
    {
      std::lock_guard lock(mutex);
      func(data) // 误将数据引用传给了函数 func, 
                 // 将无法限制 func 内部在无保护的情况下对数据 data 进行访问
    }
    void malicious_function(some_data& protected_data)
    {
      unprotected = &protected_data;
    }
    process_data(malicious_function);
    ```

* 接口中固有的竞争条件：如下代码中如果是多线程共享的栈对象，在 ① 和 ② 之间可能有其他线程对栈进行修改，从而导致错误。此外，尽管使用内部的互斥锁可以保证一次只有一个线程可以调用栈的成员函数，调用可以交替进行，如果两个线程先分别执行 ②，再分别执行 ③ ，就会导致同一个栈顶元素被访问了两遍，而有一个元素未被访问就 pop 出去了。
...
* 既然如此，为什么不将接口设计为 pop 和 top 合二为一，直接让 pop 返回栈顶元素？原因在于，构造返回值的过程可能抛异常，弹出后未返回会导致数据丢失。比如有一个元素为 vector 的 stack，拷贝 vector 需要在堆上分配内存，如果系统负载严重或资源有限（比如 vector 有大量元素），vector 的拷贝构造函数就会抛出 [std::bad_alloc](https://en.cppreference.com/w/cpp/memory/new/bad_alloc) 异常。如果 pop 可以返回栈顶元素值，返回一定是最后执行的语句，stack 在返回前已经弹出了元素，但如果拷贝返回值时抛出异常，就会导致弹出的数据丢失（从栈上移除但拷贝失败）。因此 [std::stack](https://en.cppreference.com/w/cpp/container/stack) 的设计者将这个操作分解为 top 和 pop 两部分
* 下面思考几种把 top 和 pop 合为一步的方法。
  1. 第一种容易想到的方法是传入一个引用来获取结果值，这种方式的明显缺点是，需要构造一个栈元素类型的实例，这是不现实的，为了获取结果而临时构造一个对象并不划算，元素类型可能不支持赋值（比如用户自定义某个类型），构造函数可能还需要一些参数
       ```cpp
       std::vector<int> res;
       s.pop(res);
       ```
  2. 因为 pop 返回值时只担心该过程抛异常，第二种方案是为元素类型设置不抛异常的拷贝或移动构造函数，使用 [std::is_nothrow_copy_constructible](https://en.cppreference.com/w/cpp/types/is_copy_constructible) 和 [std::is_nothrow_move_constructible](https://en.cppreference.com/w/cpp/types/is_move_constructible)。但这种方式过于局限，只支持拷贝或移动不抛异常的类型
  3. 第三种方案是返回指向弹出元素的指针，**指针可以自由拷贝且不会抛异常**，[std::shared_ptr](https://en.cppreference.com/w/cpp/memory/shared_ptr) 是个不错的选择，但这个方案的开销太大，尤其是对于内置类型来说，比如 int 为 4 字节， `shared_ptr<int>` 为 16 字节，开销是原来的 4 倍
  4. 第四种方案是结合方案一二或者一三，比如结合方案一三实现一个线程安全的 stack
...
class ConcurrentStack {
 public:
  ...
  std::shared_ptr<T> pop() { // 方案三：通过指针返回栈顶元素
    std::lock_guard<std::mutex> l(m_);
    if (s_.empty()) {
      throw EmptyStack();
    }
...
* 之前锁的粒度（锁保护的数据量大小）太小，保护操作覆盖不周全。但并非粒度越大越好，如果锁粒度太大，过多线程请求竞争占用资源时，并发的性能就会较差
* 如果给定操作需要对多个 mutex 上锁时，就会引入一个新的潜在问题，即死锁

## 死锁

* 死锁的四个必要条件：互斥、占有且等待、不可抢占、循环等待
* 避免死锁通常建议让两个 mutex 以相同顺序上锁，总是先锁 A 再锁 B，但这并不适用所有情况。[std::lock](https://en.cppreference.com/w/cpp/thread/lock) 可以同时对多个 mutex 上锁，并且没有死锁风险，它可能抛异常，此时就不会上锁，因此要么都锁住，要么都不锁。如果锁住了一个，在尝试锁第二个时有异常抛出，就自动释放第一个锁。
...
void f(A &a, A &b, int n) {
  ...
  std::lock_guard<std::mutex> lock1(a.m_, std::adopt_lock); // adopt_lock 表示 a.m_ 已经上锁了，这里只需接过互斥锁的所有权即可。
...
{ 
  if(&lhs==&rhs) 
    return; 
  std::scoped_lock guard(lhs.m,rhs.m); // 尝试锁住两个互斥锁
...
* std::unique_lock 支持通过 std::defer_lock 参数来延迟上锁
...
* 解决死锁并不简单，[std::lock](https://en.cppreference.com/w/cpp/thread/lock) 和 [std::scoped_lock](https://en.cppreference.com/w/cpp/thread/scoped_lock) 无法获取其中的锁，此时解决死锁更依赖于开发者的能力。避免死锁有四个建议
  * 第一个避免死锁的建议是，**一个线程已经获取一个锁时就不要获取第二个**。如果每个线程只有一个锁，锁上就不会产生死锁（但除了互斥锁，其他方面也可能造成死锁，比如即使无锁，**线程间相互等待** join 也可能造成死锁）
